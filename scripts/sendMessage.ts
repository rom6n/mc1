...
export async function run(provider: NetworkProvider, args: string[]) {
    ...
    const address = Address.parse(args.length > 0 ? args[0] : await ui.input('kQBmfyvtydi0NBEofnMQHnbPcQW23jsD4SS2EnpoPU4C9EBh'));
    ...
    await myContract.sendMessageEdit(
        provider.sender(),
        toNano("0.01"),
        `TEST MESSAGE ONCHAIN ${random}`.toString(),
    );
